...
// Используем те же данные, что и в ContinentDetailView
...
struct AllCountriesView: View {
    ...
    private var allCountries: [CountryInfo] {
        let allCountriesArray = europeCountries + asiaCountries + africaCountries + 
                               northAmericaCountries + southAmericaCountries + oceaniaCountries
        return allCountriesArray.sorted { $0.name < $1.name }
    }
    
    private var alphabetSections: [(String, [CountryInfo])] {
        let grouped = Dictionary(grouping: allCountries) { country in
            ...
            VStack(spacing: 0) {
                ...
                ScrollViewReader { proxy in
                    ZStack(alignment: .trailing) {
                                ...
                                ForEach(alphabetSections, id: \.0) { letter, countries in
                                    Section(header: 
                                        ...
                                        ForEach(countries, id: \.code) { country in
                                            AllCountriesCard(country: country)
                                        }
                                    }
                                }
                            }
                            .padding(.horizontal, 20)
                            .padding(.top, 0) // Убрали отступ сверху, чтобы белый блок начинался сразу после шапки
                            .padding(.bottom, 100)
                        ...
                        VStack {
                            Spacer().frame(height: contentTopInset - 20) // Поднимаем выше
    ...
    private var headerBackground: some View {
        ZStack {
            // Градиентный фон - увеличенная высота для перекрытия текста с количеством стран
            LinearGradient(
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                VStack(spacing: 8) {
                    ...
                    Text("\(allCountries.count) \(localizationManager.localizedString("стран"))")
                        ...
                        .padding(.bottom, 8) // Добавляем отступ снизу, чтобы текст был внутри заливки
                }
                .padding(.horizontal, 20)
    ...
    // Увеличена высота шапки, чтобы заливка перекрывала текст с количеством стран
    private var headerHeight: CGFloat { 240 + safeTopInset }
    
    private var contentTopInset: CGFloat {
        max(0, headerHeight - 10) // Увеличен отступ, чтобы заливка была ниже
    }
}
